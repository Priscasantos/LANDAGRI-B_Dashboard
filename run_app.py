...
def check_dependencies():
    try:
        ...
        print("[ERRO] O pacote 'streamlit' não está instalado. Instale com: pip install streamlit")
        sys.exit(1)
...
def run_streamlit_app(no_cache_flag: bool, port: int):
    ...
    cmd = [
        ...
        "--server.fileWatcherType", "auto",  # Auto-detecta mudanças nos arquivos
        "--server.runOnSave", "true",        # Executa novamente quando salvar
        "--client.toolbarMode", "developer", # Modo desenvolvedor
        "--server.allowRunOnSave", "true"    # Permite execução automática ao salvar
    ]
...
if __name__ == "__main__":
    ...
    check_dependencies()
    print("[INFO] Acesse o dashboard em: http://localhost:{}".format(args.port))
