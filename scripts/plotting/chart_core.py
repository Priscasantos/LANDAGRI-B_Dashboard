...
CHART_CONFIG = {
    ...
    "dimensions": {
        ...
        "scatter_plot": {"base": {"width": 800, "height": 600}, "max_size": 1400},
    },
}
...
def get_display_name(row: pd.Series, default_truncate_length: int = 10) -> str:
    ...
    if name_col:
        ...
        return (
            full_name[:default_truncate_length]
            if len(full_name) > default_truncate_length
            else full_name
        )
...
def create_display_name_mapping(df: pd.DataFrame) -> dict[str, str]:
    ...
    name_col = (
        "Name" if "Name" in df.columns else "Nome" if "Nome" in df.columns else None
    )
...
def prepare_temporal_display_data(
    metadata: dict[str, Any], df_original: pd.DataFrame | None = None
) -> pd.DataFrame:
    """
    ...
    for nome, meta_data in metadata.items():
        anos_disponiveis = meta_data.get(
            "anos_disponiveis", meta_data.get("available_years", [])
        )
...
def get_standard_legend_config(
    title: str = "", orientation: str = "v", position: str = "right"
) -> dict[str, Any]:
    """
    ...
    if position == "right":
        legend_config.update({"yanchor": "top", "y": 1, "xanchor": "left", "x": 1.02})
    elif position == "top":
        ...
        legend_config.update({"yanchor": "top", "y": 1, "xanchor": "right", "x": -0.02})
...
def get_standard_colorbar_config(
    title: str = "Values",
    custom_tickvals: list | None = None,
    custom_ticktext: list | None = None,
    position: str = "right",
) -> dict[str, Any]:
    """
    ...
    colorbar_config = {
        "title": {
            ...
            "font": {
                "size": config["colorbar"][
                    "title_font_size"
                ],  # Mesmo tamanho do título da legenda
        ...
        "tickfont": {
            "size": config["colorbar"][
                "tick_font_size"
            ],  # Mesmo tamanho dos itens da legenda
            "family": config["font_family"],  # Mesma fonte global
            "color": config["font_color"],  # Mesma cor global
        },
        ...
        colorbar_config.update(
            {
                ...
                "yanchor": "bottom",
                "y": 1.02,
                "title": {**colorbar_config["title"], "side": "top"},
            }
        )
    elif position == "bottom":
        colorbar_config.update(
            {
                ...
                "yanchor": "top",
                "y": -0.1,
                "title": {**colorbar_config["title"], "side": "bottom"},
            }
        )
    elif position == "left":
        colorbar_config.update(
            {
                ...
                "yanchor": "middle",
                "y": 0.5,
                "title": {**colorbar_config["title"], "side": "left"},
            }
        )
...
def apply_standard_colorbar(
    ...
    """
    colorbar_config = get_standard_colorbar_config(
        title, custom_tickvals, custom_ticktext, position
    )
...
def apply_standard_legend(
    fig, title: str = "", position: str = "right", show_legend: bool = True
) -> None:
    """
...
def get_standard_dimensions(
    ...
    if (
        chart_type in config
        and isinstance(config[chart_type], dict)
        and "base" in config[chart_type]
    ):
...
def apply_standard_dimensions(
    ...
    fig.update_layout(width=dimensions["width"], height=dimensions["height"])


def apply_heatmap_with_performance_title(
    fig,
    custom_tickvals: list | None = None,
    custom_ticktext: list | None = None,
    ...
    colorbar_margins = {"bottom": 80, "left": 200, "right": 200, "top": 60}
    ...
    fig.update_layout(
        xaxis={"tickangle": 0, "side": "bottom"},
        yaxis={"type": "category", "autorange": "reversed", "side": "left"},
    )
