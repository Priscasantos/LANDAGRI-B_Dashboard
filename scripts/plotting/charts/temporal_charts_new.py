...
from scripts.plotting.chart_core import apply_standard_layout, add_display_names_to_df
from scripts.utilities.config import get_initiative_color_map

@cache_figure(ttl=600)
def plot_timeline_chart(metadata: Dict[str, Any], filtered_df: pd.DataFrame) -> go.Figure:
    """
        ...
        if anos_disponiveis:
            ...
            for seg_start, seg_end in segments:
                fig_timeline.add_trace(go.Scatter(
                    ...
                    y=[acronym, acronym], 
                    mode='lines',
                    line=dict(color=cor, width=12),  # Standardized line width
                    name=acronym,
                    showlegend=False,
                    legendgroup=acronym,
    ...
    fig_timeline.update_layout(
        ...
        margin=dict(l=200, r=40, t=60, b=80),  # Standardized margins
        yaxis=dict(
            tickmode='array',
...
def add_chart_download(fig: go.Figure, default_filename: str, key_prefix: str):
    ...
    """
    if fig and st.button(f"📥 Download {default_filename}", key=f"{key_prefix}_download"):
        st.success("Chart ready for download! Use browser context menu to save.")
