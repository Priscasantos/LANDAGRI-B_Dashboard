...
def plot_annual_coverage_multiselect(
    metadata: dict[str, Any], filtered_df: pd.DataFrame, selected_initiatives: list[str]
) -> go.Figure:
    ...
    if not selected_initiatives or filtered_df is None or filtered_df.empty:
        fig = go.Figure()
        fig.update_layout(
            title="Annual Coverage (No initiative selected or insufficient data)"
        )
        return fig

    # Create name to display name mapping for selected initiatives
    nome_to_display_name = {
        row["Name"]: get_display_name(row)
        for _, row in filtered_df[
            filtered_df["Name"].isin(selected_initiatives)
        ].iterrows()
    }
    ...
    for nome_original in selected_initiatives:
        if (
            nome_original not in filtered_df["Name"].values
        ):  # Ensure initiative is in filtered_df to get metadata
            continue

        meta = metadata.get(nome_original, {})
        years_key = (
            "available_years" if "available_years" in meta else "anos_disponiveis"
        )
        if years_key in meta and meta[years_key]:
            ...
            display_name = nome_to_display_name.get(
                nome_original, nome_original
            )  # Fallback to original if not mapped by selection
            for ano in anos:
                data.append(
                    {
                        "Name_original": nome_original,
    ...
    if not data:
        fig = go.Figure()
        fig.update_layout(
            title="Annual Coverage (No temporal data available for selected initiatives)"
        )
        return fig
    ...
    for nome_original_metadata, meta_content in metadata.items():
        ...
        if years_key in meta_content and meta_content[years_key]:
            ...
            for ano in valid_years:
                all_anos_data.append(
                    {"Ano": ano, "Name": nome_original_metadata, "Type": tipo}
                )

    if not all_anos_data:
        return go.Figure().update_layout(
            title="Number of Initiatives Overlap (No temporal data)"
        )
...
def plot_heatmap(metadata: dict[str, Any], filtered_df: pd.DataFrame) -> go.Figure:
    ...
    if filtered_df is None or filtered_df.empty or not metadata:
        return go.Figure().update_layout(
            title="Annual Coverage Heatmap (Insufficient data)"
        )
    ...
    for nome_original_metadata, meta_content in metadata.items():
        ...
        if years_key in meta_content and meta_content[years_key]:
            ...
            for ano in valid_years:
                all_anos_data.append(
                    {"Ano": ano, "Display_Name": display_name, "Value": 1}
                )

    if not all_anos_data:
        return go.Figure().update_layout(
            title="Annual Coverage Heatmap (No temporal data)"
        )

    all_anos_df = pd.DataFrame(all_anos_data)
    pivot = all_anos_df.pivot_table(
        index="Display_Name",
        columns="Ano",
        values="Value",
        aggfunc="count",
    ...
    if pivot.empty:
        return go.Figure().update_layout(
            title="Annual Coverage Heatmap (Pivot table is empty)"
        )
    ...
    fig.update_layout(
        ...
        xaxis={
            ...
            "tickangle": 45,
        },  # Ensure integer years, tick every 2 years
        yaxis={"type": "category"},
        coloraxis_colorbar={"title": "Available"},
    )
    return fig
