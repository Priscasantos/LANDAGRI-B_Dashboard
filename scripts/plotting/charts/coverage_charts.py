...
def plot_annual_coverage_multiselect(metadata: Dict[str, Any], filtered_df: pd.DataFrame, selected_initiatives: List[str]) -> go.Figure:
    ...
    if not df_anos.empty:
        ...
        fig.update_xaxes(
            ...
            tick0=min_year,
            dtick=1,
            range=[min_year - 0.5, max_year + 0.5],            title='Year',
            showgrid=True,
            gridcolor="#E2E8F0",
            tickformat='d',
            tickangle=45,
...
def plot_ano_overlap(metadata: Dict[str, Any], filtered_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis=dict(tickformat='d', tickangle=45)) # Ensure year is displayed as integer
    return fig
...
def plot_heatmap(metadata: Dict[str, Any], filtered_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(tickformat='d', dtick=2, range=[chart_min_year - 0.5, chart_max_year + 0.5], tickangle=45), # Ensure integer years, tick every 2 years
