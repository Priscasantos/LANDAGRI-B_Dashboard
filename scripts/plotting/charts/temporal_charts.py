...
from scripts.utilities.table_charts import create_brazilian_regions_table, create_mesoregions_table, create_custom_table
...
def plot_timeline_chart(metadata: Dict[str, Any], filtered_df: pd.DataFrame, 
    ...
    all_years_range = list(range(min_year_data, max_year_data + 1))    # Define the specific order based on the provided image
    desired_order = [
        'CGLS', 'GDW', 'ESRI-10m LULC', 'UMD-GLC', 'GPW', 'UMD-SASM', 'ESA-WC',
        'WorldCereal', 'PRODES', 'DETER', 'TerraClass Amazon', 'TerraClass Cerrado',
        'MapBiomas', 'IBGE-MLCU', 'CONAB-AM'
    ]
    ...
    for nome_original_metadata, meta_content in metadata.items():
        ...
        if not initiative_row_series.empty:
            ...
            coverage = meta_content.get('coverage', 'N/A')
            unique_items.append((nome_original_metadata, {'coverage': coverage, 'acronym': acronym}))

    # Sort items based on the desired order from the image (reversed from top to bottom)
    try:
        sorted_items = sorted(
            unique_items,
            key=lambda x: desired_order.index(x[1]['acronym']) if x[1]['acronym'] in desired_order else len(desired_order),
            reverse=True
        )
    ...
    for item in sorted_items:
        ...
        produto_data_for_matrix_rows = timeline_df[timeline_df['produto'] == nome_original_metadata]
    ...
    for acronym in display_acronyms_sorted:
        ...
        if produto_data_plot.empty:
            continue
        anos_disponiveis = sorted(produto_data_plot[produto_data_plot['disponivel'] == 1]['ano'].tolist())
        metodologia = produto_data_plot['metodologia'].iloc[0]
        original_name_for_color_key = produto_data_plot['produto'].iloc[0]
        cor = color_map.get(original_name_for_color_key, px.colors.qualitative.Set1[display_acronyms_sorted.index(acronym) % len(px.colors.qualitative.Set1)])
    ...
    fig_timeline.update_layout(
        margin=dict(l=margins['l'], r=20, t=margins['t'], b=margins['b'])  # Reduced right margin
    )
    return fig_timeline
...
def timeline_with_controls(metadata: Dict[str, Any], filtered_df: pd.DataFrame):
    ...
    if fig:
        setup_download_form(fig, 
                            default_filename="timeline_chart", 
                            key_prefix="timeline_download")
...
def plot_evolution_line_chart(temporal_data: pd.DataFrame) -> go.Figure:
    ...
    apply_standard_layout(fig, "Year", "Number of Active Initiatives",
                         chart_type="line_chart", num_items=len(years_df))
...
def plot_evolution_heatmap_chart(metadata: Dict[str, Any], filtered_df: pd.DataFrame) -> go.Figure:
    ...
    apply_standard_layout(fig, "Year", "Number of Initiatives",chart_type="line_chart")    # Customize layout for modern area chart
...
def display_brazilian_geographic_tables():
    ...
    with tab2:
        ...
        st.info("💡 **Nota**: Esta tabela apresenta exemplos selecionados de mesorregiões de diferentes estados brasileiros para demonstração.")
