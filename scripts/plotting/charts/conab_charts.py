...
def plot_conab_spatial_temporal_distribution(conab_data: Dict[str, Any]) -> go.Figure:
    ...
    for crop in crop_types:
        ...
        for state in states_list:
            ...
            for year in years:
                ...
                bar_offset = start_offset + (crop_index * subdivided_bar_height) + (subdivided_bar_height / 2)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(width=0, color=CROP_COLOR_MAP.get(crop, '#808080')),
                    name=crop,
                    legendgroup=crop,
    ...
    from scripts.plotting.chart_core import get_standard_legend_config, get_standard_bar_config
    ...
    fig.update_layout(legend_title=dict(font=dict(size=22)))
    return fig
...
def plot_conab_spatial_coverage(conab_data: Dict[str, Any]) -> go.Figure:
    ...
    apply_standard_layout(fig, "Coverage (%)", "Region", 
                         chart_type="bar_chart", num_items=len(states))
...
def plot_conab_crop_diversity(conab_data: Dict[str, Any]) -> go.Figure:
    ...
    crop_types = [c for c in CROP_ORDER if any(c in crops for crops in state_crops.values())]
    legend_added = set()
    ...
    for crop in crop_types:
        for state in states_list:
            ...
            if state in state_crops:
                count = state_crops[state].count(crop) if crop in state_crops[state] else 0
                if count > 0:
                    show_in_legend = crop not in legend_added
                    if show_in_legend:
                        legend_added.add(crop)
                    fig.add_trace(go.Bar(
                        ...
                        legendgroup=crop,
                        showlegend=show_in_legend,
                        marker=dict(color=CROP_COLOR_MAP.get(crop, '#808080'), line=dict(width=0)),
    ...
    if crop_types:  # Only if there are crops to show
        fig.add_trace(go.Bar(
            ...
            orientation='h',
            name='Sugar cane mill',
            marker=dict(color=CROP_COLOR_MAP.get('Sugar cane mill', '#808080'), line=dict(width=0)),
            showlegend=True,
...
def load_brazil_shapefile() -> Optional[gpd.GeoDataFrame]:
    """Load Brazil states shapefile."""
    try:
        current_dir = Path(__file__).parent.parent.parent.parent
        shapefile_path = current_dir / "data" / "brazil-vector" / "BR_Municipios_2023.shp"
...
def create_conab_crop_map(conab_data: Dict[str, Any], selected_crop: str = "Soybean") -> folium.Map:
    """
    ...
    state_mapping = {v: k for k, v in BRAZILIAN_STATES.items()}
    ...
    if state_col:
        ...
        for idx, row in states_gdf.iterrows():
            ...
            if state_code in state_data:
                ...
                states_gdf.loc[idx, 'coverage_info'] = f"{coverage} seasons: {years_str}"
        ...
        for idx, row in states_gdf.iterrows():
            ...
            folium.GeoJson(
                ...
                tooltip=folium.Tooltip(
                    f"{row[state_col]}: {coverage} seasons"
                )
            ).add_to(m)
...
def create_conab_all_crops_map(conab_data: Dict[str, Any]) -> folium.Map:
    ...
    for crop, color in CROP_COLOR_MAP.items():
        if crop in detailed_coverage and crop != "Sugar cane mill":
            legend_items.append(f'<i class="fa fa-circle" style="color:{color}"></i> {crop}')
...
def get_state_coordinates(state_code: str) -> Optional[List[float]]:
    """
