...
def plot_distribuicao_classes(filtered_df):
    ...
    if "Classes" not in filtered_df.columns:
        fig = go.Figure()
        fig.update_layout(
            title="Number of Classes Distribution ('Classes' column not found)"
        )
        return fig
...
def plot_distribuicao_metodologias(method_counts):
    ...
    if method_counts is None or method_counts.empty:
        fig = go.Figure()
        fig.update_layout(
            title="Distribution of Methodologies Used (Insufficient data)"
        )
        return fig
...
def plot_resolution_accuracy(filtered_df: pd.DataFrame) -> go.Figure:
    ...
    if (
        "Resolution (m)" not in filtered_df.columns
        or "Accuracy (%)" not in filtered_df.columns
    ):
        fig = go.Figure()
