...
def plot_resolution_accuracy_scatter(filtered_df: pd.DataFrame) -> go.Figure:
    ...
    apply_standard_layout(fig, 'Resolution (m)', 'Accuracy (%)',
...
def plot_normalized_performance_heatmap(filtered_df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        hovertemplate="<b>Initiative:</b> %{y}<br>" +
                      ...
                      "<b>Rating:</b> <b>%{z|.0%}</b><extra></extra>",        zmin=0,
        zmax=1
    ))
    ...
    apply_heatmap_with_performance_title(
        ...
        custom_ticktext=['<b>Poor</b>', '<b>Below Average</b>', '<b>Average</b>', '<b>Good</b>', '<b>Excellent</b>'],
        xaxis_title="Metrics",
...
def plot_initiative_ranking(filtered_df: pd.DataFrame, ranking_criteria: Dict[str, float] | None = None) -> go.Figure:
    ...
    # Apply standardized layout
    apply_standard_layout(fig, "Composite Score", "Initiative",
                         chart_type="bar_chart", num_items=len(plot_df))
...
def plot_correlation_matrix(filtered_df: pd.DataFrame) -> go.Figure:
    ...
    # Apply standardized layout
    apply_standard_layout(fig, "Variables", "Variables", 
                         chart_type="correlation_matrix", num_items=len(numeric_columns),
                         show_legend=False)
...
def add_comparison_chart_download(fig: go.Figure, default_filename: str, key_prefix: str):
    """
