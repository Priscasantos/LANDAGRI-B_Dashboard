...
def load_data_from_jsonc(jsonc_path: str = None) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Load data from JSONC using the unified processor."""
    processor = UnifiedDataProcessor()
    return processor.load_data_from_jsonc(jsonc_path)

def prepare_plot_data(df: pd.DataFrame) -> pd.DataFrame:
    """Prepare data for plotting by standardizing columns and handling missing values."""
    processor = UnifiedDataProcessor()
    ...
    # Standardize columns
    df_prepared = standardize_dataframe_columns(df_prepared)
    
    # Handle missing values
    for col in df_prepared.columns:
        if df_prepared[col].dtype == 'object':
            df_prepared[col] = df_prepared[col].fillna('N/A')
        else:
            df_prepared[col] = df_prepared[col].fillna(0)
    
    # Add any derived columns that might be needed for plotting
    if 'resolucao' in df_prepared.columns:
        df_prepared['categoria_resolucao'] = df_prepared['resolucao'].apply(processor.categorize_resolution)
    
    if 'acuracia' in df_prepared.columns:
        df_prepared['categoria_acuracia'] = df_prepared['acuracia'].apply(processor.categorize_accuracy)
    
    if 'provedor' in df_prepared.columns:
        df_prepared['categoria_provedor'] = df_prepared['provedor'].apply(processor.categorize_provider)
    
    if 'metodologia' in df_prepared.columns:
        df_prepared['categoria_metodologia'] = df_prepared['metodologia'].apply(processor.categorize_methodology)
