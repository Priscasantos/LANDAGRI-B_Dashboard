...
class UnifiedDataProcessor:
    ...
    def parse_accuracy(self, accuracy_value: str | int | float | dict) -> float:
        ...
        if not accuracy_value or accuracy_value in [
            "Not informed",
            "Incomplete",
            "N/A",
            "Not available",
    ...
    def _parse_enhanced_resolution(self, resolution_value):
        ...
        if isinstance(resolution_value, list):
            ...
            for res_obj in resolution_value:
                if isinstance(res_obj, dict) and res_obj.get("current", False):
                    return self._parse_enhanced_resolution(
                        res_obj.get("resolution", 30)
                    )

            # If no current resolution, use the first one
            if resolution_value and isinstance(resolution_value[0], dict):
                return self._parse_enhanced_resolution(
                    resolution_value[0].get("resolution", 30)
                )
    ...
    def parse_resolution(self, resolution_value: str | int | float | list) -> float:
    ...
    def parse_reference_system(self, reference_system_value: str | list) -> str:
    ...
    def get_accuracy_details(
        self, accuracy_value: str | int | float | dict
    ) -> dict[str, Any]:
        ...
        if isinstance(accuracy_value, dict):
            if "status" in accuracy_value:
                ...
                details["overall"] = self.parse_accuracy(
                    accuracy_value.get("overall", 0)
                )
    ...
    def parse_temporal_data(
        self, temporal_interval: list[int] | str | None
    ) -> dict[str, Any]:
        ...
        if temporal_interval is None:
            ...
            try:
                years = [
                    int(y.strip())
                    for y in temporal_interval.split(",")
                    if y.strip().isdigit()
                ]
            ...
            for y in temporal_interval:
                if isinstance(y, int | float) or (
                    isinstance(y, str) and y.strip().isdigit()
                ):
        ...
        return {
            ...
            "available_years_str": ",".join(
                map(str, years)
            ),  # String version for display/CSV
    ...
    def categorize_provider(self, provider: str) -> str:
        ...
        if any(
            ...
            for term in ["space", "esa", "copernicus", "nasa", "inpe"]
        ):
            return "Space Agency"
            ...
            term in provider_lower
            for term in ["government", "institute", "ibge", "conab", "embrapa"]
        ):
            return "Government"
    ...
    def load_data_from_jsonc(
        self, jsonc_path: str | None = None
    ) -> tuple[pd.DataFrame, dict[str, Any]]:
        """Unified data loading from JSONC with comprehensive processing."""

        if jsonc_path is None:
            jsonc_path = "data/json/initiatives_metadata.jsonc"
        ...
        try:
            ...
            raise FileNotFoundError(f"Metadata file not found: {jsonc_path}") from exc
        ...
        for initiative_name, initiative_data in metadata.items():
            ...
            resolution = self.parse_resolution(
                initiative_data.get("spatial_resolution", 30)
            )  # Enhanced accuracy parsing - support both old and new formats
            accuracy_raw = initiative_data.get(
                "overall_accuracy", initiative_data.get("accuracy", 0)
            )
            ...
            row = {
                ...
                "Type": self.categorize_coverage(
                    initiative_data.get("coverage", "Regional")
                ),
                "Scope": initiative_data.get("coverage", "Regional"),
                "Provider": initiative_data.get("provider", ""),
                "Provider Type": self.categorize_provider(
                    initiative_data.get("provider", "")
                ),
                ...
                "Classes": (
                    int(final_classes) if isinstance(final_classes, int | float) else 1
                ),
                ...
                "Available Years": temporal_info[
                    "available_years_str"
                ],  # Keep as string for DataFrame
                "Temporal Gaps": (
                    ",".join(map(str, temporal_info["temporal_gaps"]))
        ...
        for initiative_name, initiative_data in metadata.items():
            ...
            enhanced_data.update(
                {
                    "acronym": initiative_data.get("acronym", initiative_name[:8]),
                    "available_years": temporal_info_meta[
                        "available_years"
    ...
    def create_comparison_matrix(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        normalized_cols = [
            col for col in comparison_data.columns if col.endswith("_normalized")
        ]
        comparison_data["Overall_Score"] = comparison_data[normalized_cols].mean(axis=1)

        return comparison_data

    def create_temporal_analysis_data(
        ...
        for name, meta in metadata.items():
            ...
            acronym = name_to_acronym.get(
                name, name[:10]
            )  # Simplified, relies on df for acronym if available, otherwise truncates name
            years = sorted(years)
            ...
            if temporal_info["temporal_gaps"]:
                for gap_year in temporal_info["temporal_gaps"]:
                    initiative_data["gaps"].append({"year": gap_year, "duration": 1})
        ...
        for init_data in temporal_data["initiatives"]:
            ...
            availability = [
                1 if year in init_data["years"] else 0 for year in all_years_sorted
            ]
        ...
        for init_data in temporal_data["initiatives"]:
            ...
            coverage_percentage = (
                (init_data["total_years"] / total_span) * 100 if total_span > 0 else 0
            )
            ...
            temporal_data["coverage_stats"]["by_initiative"][acronym] = {
                ...
                "efficiency": (
                    (init_data["total_years"] / init_data["coverage_span"]) * 100
                    if init_data["coverage_span"] > 0
                    else 100
                ),
            }
    ...
    def create_comprehensive_auxiliary_data(
        ...
        auxiliary_data = {
            ...
            "data_summary": {
                ...
                "initiatives_with_temporal_data": sum(
                    1 for item in metadata.values() if item.get("available_years")
        ...
        print(
            f"✅ Generated auxiliary data for {auxiliary_data['data_summary']['total_initiatives']} initiatives"
        )
        print("📊 Using unified mappings for consistency")

        return auxiliary_data

    def create_optimized_auxiliary_data(
        self, df: pd.DataFrame, metadata: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        optimized_data = {
            ...
            "temporal_analysis_v2": self._create_optimized_temporal_analysis(
                standard_data.get("temporal_analysis", {})
            ),
            "visualization_ready_data": self._create_visualization_ready_data(
                df, metadata
            ),
        ...
        print(
            f"🎯 Visualization ready: {len(optimized_data['visualization_ready_data']['chart_types'])} chart types"
        )
    ...
    def _create_compact_comparison_matrix(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        for _, row in df.iterrows():
            ...
            compact_metrics[acronym] = {
                ...
                "acc_cat": self._categorize_accuracy_compact(
                    float(row.get("Accuracy (%)", 0))
                ),
                "res_cat": self._categorize_resolution_compact(
                    float(row.get("Resolution (m)", 0))
                ),
        ...
        return {
            ...
            "stats": {
                ...
                "acc_range": (
                    [min(accuracies), max(accuracies)] if accuracies else [0, 0]
                ),
                "res_range": (
                    [min(resolutions), max(resolutions)] if resolutions else [0, 0]
                ),
    ...
    def _create_optimized_temporal_analysis(
        ...
        for year in range(min_year, max_year + 1):
            ...
            for data in optimized_temporal.values():
                ...
                if (
                    year_index < len(data["bitmap"])
                    and data["bitmap"][year_index] == "1"
                ):
        ...
        return {
            ...
            "overlap_stats": {
                ...
                "avg_concurrent": (
                    np.mean(list(year_coverage.values())) if year_coverage else 0
                ),
    ...
    def _create_visualization_ready_data(
        self, df: pd.DataFrame, metadata: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if acronym_col in df.columns and name_col in df.columns:
            name_to_acronym_df = pd.Series(
                df[acronym_col].values, index=df[name_col]
            ).to_dict()
                ...
                initiative_temporal_data.append(
                    {
                        ...
                        "acronym": name_to_acronym_df.get(
                            name, name[:10]
                        ),  # Use mapping from df or truncate name
        ...
        if all_years_set:
            ...
            for init_data in initiative_temporal_data:
                availability = [
                    1 if year in init_data["years"] else 0 for year in sorted_years
                ]
        ...
        if vis_data["temporal_evolution_data"]:
            ...
            if heatmap_df_data:
                vis_data["heatmap_data"] = (
                    pd.DataFrame(heatmap_df_data)
                    .pivot_table(
                        index="acronym",
                        columns="year",
                        values="available",
    ...
    def _calculate_performance_metrics(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        metrics = {
            "accuracy_distribution": (
                df["Accuracy (%)"].describe().to_dict()
                if "Accuracy (%)" in df.columns
                else {}
            ),
            "resolution_distribution": (
                df["Resolution (m)"].describe().to_dict()
                if "Resolution (m)" in df.columns
                else {}
            ),
            "provider_diversity": (
                df["Provider Type"].nunique() if "Provider Type" in df.columns else 0
            ),
            "methodology_diversity": (
                df["Method Category"].nunique()
                if "Method Category" in df.columns
                else 0
            ),
            "temporal_coverage": {
                "avg_span": (
                    df["Temporal Span"].mean() if "Temporal Span" in df.columns else 0
                ),
                "max_span": (
                    df["Temporal Span"].max() if "Temporal Span" in df.columns else 0
                ),
                "min_span": (
                    df["Temporal Span"].min() if "Temporal Span" in df.columns else 0
                ),
            },
        }
    ...
    def _calculate_key_correlations(self, df: pd.DataFrame) -> dict[str, float]:
        ...
        if "Accuracy (%)" in df.columns and "Resolution (m)" in df.columns:
            ...
            correlations["accuracy_vs_resolution"] = (
                float(corr) if not pd.isna(corr) else 0
            )
    ...
    def _identify_outliers(self, df: pd.DataFrame) -> dict[str, list[str]]:
        ...
        if "Accuracy (%)" in df.columns:
            ...
            outliers["high_accuracy"] = [
                str(row[acronym_col])[:10] for _, row in high_outliers.iterrows()
            ]
    ...
    def _generate_recommendations(self, df: pd.DataFrame) -> list[str]:
        ...
        if "Accuracy (%)" in df.columns:
            ...
            if avg_accuracy < 80:
                recommendations.append(
                    "Consider focusing on high-accuracy initiatives for better results"
                )
        ...
        if "Resolution (m)" in df.columns:
            ...
            if high_res_count < len(df) * 0.3:
                recommendations.append(
                    "More high-resolution initiatives needed for detailed analysis"
                )

        return recommendations

    def _estimate_optimized_size(
        self, df: pd.DataFrame, metadata: dict[str, Any]
    ) -> dict[str, float]:
            ...
            if data_type.upper() == "JSON":
                ...
                def convert_df_to_dict_recursive(item):
                    if isinstance(item, pd.DataFrame):
                        ...
                        return {
                            k: convert_df_to_dict_recursive(v) for k, v in item.items()
                        }
                ...
                with open(filepath, "w", encoding="utf-8") as f:
                    json.dump(
                        data_to_save, f, ensure_ascii=False, indent=2, default=str
                    )
    ...
    def validate_data(
        ...
        if df is None or df.empty:
                ...
                if col in df.columns:
                    ...
                    if null_count > 0:
                        validation_results["issues"].append(
                            f"Null values in {col}: {null_count}"
                        )

            # Check data ranges
            if "Accuracy (%)" in df.columns:
                invalid_accuracy = df[
                    (df["Accuracy (%)"] < 0) | (df["Accuracy (%)"] > 100)
                ]
                if not invalid_accuracy.empty:
                    validation_results["issues"].append(
                        f"Invalid accuracy values: {len(invalid_accuracy)} rows"
                    )
            ...
            if "Resolution (m)" in df.columns:
                ...
                if not invalid_resolution.empty:
                    validation_results["issues"].append(
                        f"Invalid resolution values: {len(invalid_resolution)} rows"
                    )
        ...
        if not metadata:
            ...
            for name, data in metadata.items():
                ...
                if not isinstance(metadata_years, list):
                    validation_results["issues"].append(
                        f"Invalid 'available_years' format for {name}: not a list"
                    )
                else:
                    for year in metadata_years:
                        if not isinstance(year, int):
                            validation_results["issues"].append(
                                f"Invalid year in 'available_years' for {name}: {year} is not an integer"
                            )
                            break

        # Set overall validity
        validation_results["dataframe_valid"] = (
            len(
                [
                    i
                    for i in validation_results["issues"]
                    if "DataFrame" in i
                    or "column" in i
                    or "accuracy" in i
                    or "resolution" in i
        ...
        validation_results["metadata_valid"] = (
            len(
                [
                    i
                    for i in validation_results["issues"]
                    if "Metadata" in i or "mismatch" in i
                ]
            )
            == 0
        )
        ...
        validation_results["summary"] = {
            ...
            "validation_passed": validation_results["dataframe_valid"]
            and validation_results["metadata_valid"],
        }
...
def generate_all_auxiliary_data(
...
def save_auxiliary_data(
    auxiliary_data: dict[str, Any], filepath: str | None = None
) -> bool:
...
if __name__ == "__main__":
    ...
    try:
        ...
        processor.save_data(
            auxiliary_data, "data/processed/unified_auxiliary_data.json", "JSON"
        )
