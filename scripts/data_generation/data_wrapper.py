...
class DataWrapper:
    ...
    def prepare_plot_data(self, df: pd.DataFrame, plot_type: str = 'comparison', 
                         filter_params: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        if current_df.empty:
            return {'error': 'No data available', 'data': pd.DataFrame()} # Ensure a DataFrame is always in 'data' key
            
        plot_data = {'plot_type': plot_type, 'data': pd.DataFrame()} # Initialize with an empty DataFrame
        ...
        try:
            if plot_type == 'comparison':
                ...
                plot_data['warning'] = f'Unknown plot_type: {plot_type}. Returning general data.'
    ...
    def _prepare_comparison_data(self, df: pd.DataFrame, auxiliary: Dict) -> Dict[str, Any]:
    ...
    def _prepare_temporal_data(self, df: pd.DataFrame, auxiliary: Dict) -> Dict[str, Any]:
    ...
    def _prepare_detailed_data(self, df: pd.DataFrame, metadata: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary_stats': {
                ...
                'coverage_types': df['Coverage'].unique().tolist() if 'Coverage' in df.columns else [],
                'resolution_stats': self._get_resolution_stats(df),
                'accuracy_stats': self._get_accuracy_stats(df)
            }
        }
    
    def _prepare_overview_data(self, df: pd.DataFrame, auxiliary: Dict) -> Dict[str, Any]:
        """Prepare data for overview dashboard."""
        return {
            'summary': {
                'total_initiatives': len(df),
                'global_initiatives': len(df[df['Coverage'] == 'Global']) if 'Coverage' in df.columns else 0,
                'national_initiatives': len(df[df['Coverage'] == 'National']) if 'Coverage' in df.columns else 0,
                'regional_initiatives': len(df[df['Coverage'] == 'Regional']) if 'Coverage' in df.columns else 0,
            },
    ...
    def _get_top_providers(self, df: pd.DataFrame, top_n: int = 5) -> List[Dict]:
        ...
        return [{'provider': provider, 'count': int(count)} 
    ...
    def _get_resolution_distribution(self, df: pd.DataFrame) -> List[Dict]:
        ...
        df_copy['Resolution Category'] = pd.cut(
            ...
            bins=[0, 15, 35, 65, 200],
            labels=['Very High (≤15m)', 'High (16-35m)', 'Medium (36-65m)', 'Low (>65m)'],
            include_lowest=True
        )
        
        dist = df_copy['Resolution Category'].value_counts()
        return [{'category': str(cat), 'count': int(count)} 
    ...
    def _get_accuracy_distribution(self, df: pd.DataFrame) -> List[Dict]:
        ...
        df_filtered['Accuracy Category'] = pd.cut(
            ...
            bins=[0, 70, 80, 90, 100],
            labels=['Fair (≤70%)', 'Good (71-80%)', 'Very Good (81-90%)', 'Excellent (>90%)'],
            include_lowest=True
        )
...
def prepare_plot_data(df: pd.DataFrame, plot_type: str = 'default', 
                        filter_params: Optional[Dict] = None) -> Dict[str, Any]:
