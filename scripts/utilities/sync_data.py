                ...
                try:
                    ...
                    if "-" in anos_str and "," not in anos_str:
                        ...
                        processed_meta["anos_disponiveis"] = [
                            int(x.strip()) for x in anos_str.split(",")
                        ]
...
def create_csv_from_metadata(metadata_dict):
    ...
    for name, meta in metadata_dict.items():
        ...
        try:
            resolucao = float(
                resolucao_str.replace("m", "").replace("-", "").split()[0]
            )
...
def main():
    ...
    with open("initiative_data/initiative_meta.json", encoding="utf-8") as f:
    ...
    df_updated.to_csv(
        "initiative_data/initiatives_processed.csv", index=False, encoding="utf-8"
    )
    ...
    print(
        f"   - Iniciativas globais: {len(df_updated[df_updated['Tipo'] == 'Global'])}"
    )
    print(
        f"   - Iniciativas nacionais: {len(df_updated[df_updated['Tipo'] == 'Nacional'])}"
    )
    print(
        f"   - Iniciativas regionais: {len(df_updated[df_updated['Tipo'] == 'Regional'])}"
    )
    print(
        f"   - Acurácia média: {df_updated[df_updated['Acurácia (%)'] > 0]['Acurácia (%)'].mean():.1f}%"
    )
