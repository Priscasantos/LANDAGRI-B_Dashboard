...
class DataOptimizer:
    ...
    def optimize_initiatives_data(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        optimized["unique_sensors"] = (
            df["Sensor"].nunique() if "Sensor" in df.columns else 0
        )
        optimized["unique_countries"] = (
            ...
            if date_col in df.columns:
                try:
                    ...
                    optimized[f"{date_col}_range"] = {
                        "min": (
                            df_temp[date_col].min().isoformat()
                            if pd.notna(df_temp[date_col].min())
                            else None
                        ),
                        "max": (
                            df_temp[date_col].max().isoformat()
                            if pd.notna(df_temp[date_col].max())
                            else None
                        ),
                    }
                except Exception:
        ...
        for col in categorical_columns:
            if (
                col in df.columns and df[col].nunique() < 100
            ):  # Evita colunas com muitos valores únicos
    ...
    def _create_paginated_chunks(
        self, df: pd.DataFrame, chunk_size: int = 50
    ...
    def invalidate_optimization(self, pattern: str | None = None):
...
def load_and_optimize_initiatives(
    file_path: str | Path,
    ...
    """
    # Carrega dados usando o parser JSONC
    try:
        from .json_interpreter import interpret_initiatives_metadata

        df = interpret_initiatives_metadata(file_path)
    except ImportError:
        # Fallback para carregamento tradicional
        try:
            from .json_interpreter import _load_jsonc_file

            if isinstance(file_path, str):
                file_path = Path(file_path)
            data = _load_jsonc_file(file_path)
...
def preload_all_data():
    ...
    try:
        ...
        initiatives_file = "data/json/initiatives_metadata.jsonc"
        sensors_file = "data/json/sensors_metadata.jsonc"
        ...
        st.success(
            f"✅ Dados otimizados carregados! Hit Rate: {cache_stats['hit_rate_percent']}%"
        )
