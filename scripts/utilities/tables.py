...
def gap_analysis(metadata: dict[str, Any], filtered_df: pd.DataFrame) -> pd.DataFrame:
            ...
            gap_data.append(
                {
                    ...
                    "First Year": meta_info.get(
                        "first_year"
                    ),  # Changed 'primeiro_ano' to 'first_year'
                    "Last Year": meta_info.get(
                        "last_year"
                    ),  # Changed 'ultimo_ano' to 'last_year'
                    "Number of years with temporal gap": meta_info.get(
                        "years_with_gap", 0
                    ),  # Changed 'anos_com_lacuna' to 'years_with_gap'
                    "Largest temporal gap": meta_info.get(
                        "largest_gap", 0
                    ),  # Changed 'maior_lacuna' to 'largest_gap'
                    "Type": meta_info.get(
                        "type", "Unknown"
                    ),  # Changed 'tipo' to 'type' and 'Desconhecido' to 'Unknown'
                }
            )
    ...
    return gap_df[
        gap_df["First Year"].notna()
    ].copy()  # Changed 'Primeiro Ano' to 'First Year'
