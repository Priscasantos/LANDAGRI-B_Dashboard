...
def _load_jsonc_file(file_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        if lines:  # Check if lines was populated
            error_line_content = (
                "".join(lines).splitlines()[e.lineno - 1]
                if hasattr(e, "lineno") and e.lineno <= len(lines)
                else "Error line out of bounds or not available"
            )
            ...
            for line_num, line_content_iter in enumerate(lines):
                if not line_content_iter.strip().startswith("//"):
                    try:
                        ...
                        if (
                            line_content_iter.strip().endswith(",")
                            or line_content_iter.strip().endswith("{")
                            or line_content_iter.strip().endswith("[")
                            or ":" in line_content_iter
                        ):
                            cleaned_lines_for_fallback.append(line_content_iter)
                    except Exception:
                        print(
                            f"Skipping potentially problematic line {line_num + 1}: {line_content_iter.strip()}"
                        )
        try:
...
def _parse_single_resolution(res_value: Any) -> float | None:
...
def parse_resolution(resolution_field: Any) -> dict[str, float | None]:
    """
...
def _parse_single_accuracy(acc_value: Any) -> float | None:
...
def parse_accuracy(accuracy_field: Any) -> dict[str, float | None]:
    """
    ...
    if isinstance(accuracy_field, dict):
        ...
        if "by_product" in accuracy_field and isinstance(
            accuracy_field["by_product"], list
        ):
        ...
        if (
            not accuracies
            and "by_collection" in accuracy_field
            and isinstance(accuracy_field["by_collection"], list)
        ):
            current_collection_acc = None
            for coll in accuracy_field["by_collection"]:
                if isinstance(coll, dict) and coll.get("current", False):
                    current_collection_acc = _parse_single_accuracy(
                        coll.get("accuracy")
                    )
                    break
            if current_collection_acc is not None:
                ...
                first_coll_acc = _parse_single_accuracy(
                    accuracy_field["by_collection"][0].get("accuracy")
                )
...
def parse_reference_system(reference_system_field: Any) -> str:
    ...
    if isinstance(reference_system_field, str):
        ...
        if "," in reference_system_field:
            parsed_systems.extend(
                [s.strip() for s in reference_system_field.split(",") if s.strip()]
            )
        else:
            ...
            if isinstance(item, str):
                ...
                if epsg_code:
                    parsed_systems.append(
                        f"{epsg_code} ({description})" if description else epsg_code
                    )
                elif description:
                    parsed_systems.append(description)
    # Remove duplicates and return as comma-separated string
    unique_systems = sorted({s for s in parsed_systems if s})
    return ", ".join(unique_systems) if unique_systems else "Not specified"


def _generate_display_name(name: str, acronym: str | None) -> str:
...
def _standardize_type(coverage_field: str | None) -> str:
...
def _parse_available_years(years_field: Any) -> list[int]:
    ...
    if isinstance(years_field, list):
        return sorted(
            {
                int(y)
                for y in years_field
                if isinstance(y, int | float) or (isinstance(y, str) and y.isdigit())
            }
        )
    if isinstance(years_field, str):
        try:
            ...
            return sorted(
                {
                    int(y.strip())
                    for y in re.split(r"[\s,]+", years_field)
                    if y.strip().isdigit()
                }
            )
        except ValueError:
...
def interpret_initiatives_metadata(file_path: str | Path | None = None) -> pd.DataFrame:
    """
    ...
    for initiative_name, details in raw_data.items():
        if not isinstance(details, dict):  # Skip if details are not a dictionary
            print(
                f"Warning: Skipping initiative '{initiative_name}' due to unexpected data format: {type(details)}"
            )
            continue
        ...
        resolution_data = parse_resolution(
            _get_safe_value(details, "spatial_resolution")
        )
        accuracy_data = parse_accuracy(
            _get_safe_value(details, "overall_accuracy")
            or _get_safe_value(details, "accuracy")
        )  # Check both keys
        available_years_list = _parse_available_years(
            _get_safe_value(details, "available_years")
        )
        available_years_str = (
            f"{min(available_years_list)}-{max(available_years_list)}"
        ...
        if isinstance(class_legend_data, str):
            processed_class_legend_list = [
                s.strip() for s in class_legend_data.split(",") if s.strip()
            ]
        elif isinstance(class_legend_data, list):
            processed_class_legend_list = (
                class_legend_data  # Assumes it's already a list of strings
            )

        # New fields for agricultural classes and capability
        num_agri_classes = _get_safe_value(details, "number_of_agriculture_classes")
        capability_text = _get_safe_value(
            details, "capability"
        )  # For general capability display
        agricultural_capabilities_text = _get_safe_value(
            details, "agricultural_capabilities"
        )  # Specific for agri legend

        # Derive Agricultural_Class_Legend
        agri_legend_list = []
        if (
            isinstance(num_agri_classes, int)
            and num_agri_classes == 1
            and isinstance(agricultural_capabilities_text, str)
            and agricultural_capabilities_text.strip()
        ):
        ...
        initiative_dict = {
            ...
            ),  # Add Temporal_Frequency column
            "Temporal_Coverage_Start": _get_safe_value(
                details, "temporal_coverage_start_date"
            ),
            "Temporal_Coverage_End": _get_safe_value(
                details, "temporal_coverage_end_date"
            ),
            "Available_Years_Str": available_years_str,
            "Available_Years_List": json.dumps(
            ...
            "Class_Legend": json.dumps(
                processed_class_legend_list
            ),  # Store the processed list as JSON string
            "Agricultural_Class_Legend": json.dumps(
                agri_legend_list
            ),  # New: Derived agricultural legend
    ...
    numeric_cols = [
        ...
        "Classes",
        "Num_Agri_Classes",
    ]  # Added Num_Agri_Classes to numeric cols
...
def interpret_combined_conab_metadata(
    main_file_path: str | Path | None = None, conab_file_path: str | Path | None = None
) -> pd.DataFrame:
    """
    ...
    if not main_df.empty and not conab_df.empty:
        ...
        print(
            f"Combined {len(main_df)} main initiatives with {len(conab_df)} CONAB initiatives"
        )
        return combined_df
...
def get_conab_crop_availability(
    conab_file_path: str | Path | None = None,
) -> dict[str, Any]:
    """
...
def load_mesoregions_dictionary(
...
def get_mesoregion_by_state(
    state_code: str, mesoregions_dict: dict[str, Any] | None = None
) -> str | None:
    """
        ...
        if "states" in mesoregion_data:
            for state in mesoregion_data["states"]:
                if (
                    ...
                    and state.get("sigla", "").upper() == state_code_upper
                ):
...
def get_states_by_mesoregion(
    mesoregion_name: str, mesoregions_dict: dict[str, Any] | None = None
) -> list[dict[str, str]]:
    """
...
def enrich_conab_data_with_mesoregions(
    conab_data: dict[str, Any], mesoregions_dict: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
    ...
    if "crop_coverage" in enriched_data:
        for _crop_name, crop_data in enriched_data["crop_coverage"].items():
            if "regions" in crop_data:
                ...
                for state_code in crop_data["regions"]:
                    ...
                    mesoregion_info = (
                        get_mesoregion_info(mesoregion, mesoregions_dict)
                        if mesoregion
                        else None
                    )
                    ...
                    regions_with_mesoregions.append(
                        {
                            ...
                            "mesoregion_color": (
                                mesoregion_info.get("color")
                                if mesoregion_info
                                else None
                            ),
                            "mesoregion_pt": (
                                mesoregion_info.get("name_pt")
                                if mesoregion_info
                                else None
                            ),
                        }
                ...
                for region_info in regions_with_mesoregions:
                    ...
                    if mesoregion:
                        ...
                        mesoregion_groups[mesoregion]["states"].append(
                            region_info["state_code"]
                        )
    ...
    if "regional_coverage" in enriched_data:
                ...
                regional_coverage_with_mesoregions.append(
                    {
                        ...
                        "mesoregion": mesoregion,
                        "mesoregion_color": (
                            mesoregion_info.get("color") if mesoregion_info else None
                        ),
                        "mesoregion_pt": (
                            mesoregion_info.get("name_pt") if mesoregion_info else None
                        ),
                    }
                )
            else:
        ...
        enriched_data["regional_coverage_with_mesoregions"] = (
            regional_coverage_with_mesoregions
        )

    # Add mesoregions color palette
    enriched_data["mesoregions_palette"] = get_all_mesoregions_with_colors(
        mesoregions_dict
    )
...
def get_integrated_conab_analysis(
    conab_file_path: str | Path | None = None, dictionary_path: str | Path | None = None
) -> dict[str, Any]:
    """
    ...
    analysis = {
        ...
        "summary": {
            ...
            "total_states_covered": len(
                {
                    region["state_code"]
                    for crop_data in enriched_data.get("crop_coverage", {}).values()
                    for region in crop_data.get("regions_with_mesoregions", [])
                    if region["state_code"]
                }
            ),
            "mesoregions_covered": list(
                {
                    region["mesoregion"]
                    for crop_data in enriched_data.get("crop_coverage", {}).values()
                    for region in crop_data.get("regions_with_mesoregions", [])
    ...
    for crop_name, crop_data in enriched_data.get("crop_coverage", {}).items():
        for mesoregion, mesoregion_info in crop_data.get(
            "mesoregion_groups", {}
        ).items():
            ...
            crops_by_mesoregion[mesoregion]["crops"].append(
                {
                    "crop": crop_name,
                    "states": mesoregion_info.get("states", []),
...
def get_all_mesoregions_with_colors(
    mesoregions_dict: dict[str, Any] | None = None,
...
if __name__ == "__main__":
    ...
    test_file_path = project_root_path / "data" / "json" / "initiatives_metadata.jsonc"
