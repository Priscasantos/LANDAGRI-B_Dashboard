...
def create_custom_table(data: List[Dict[str, Any]], 
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>{title}</b>",
            x=0.5,
            font=dict(size=20, color=scheme['header'], family="Arial Black")
        ),
...
def create_summary_stats_table(data: pd.DataFrame, title: str = "Summary Statistics") -> go.Figure:
    """
    ...
    if len(numeric_cols) == 0:
        return go.Figure().add_annotation(text="No numeric columns to summarize", showarrow=False)
    ...
    for col in numeric_cols:
        summary_data.append({
            ...
            'Mean': f"{data[col].mean():.2f}" if not data[col].isna().all() else "N/A",
            'Std': f"{data[col].std():.2f}" if not data[col].isna().all() else "N/A",
            'Min': f"{data[col].min():.2f}" if not data[col].isna().all() else "N/A",
            'Max': f"{data[col].max():.2f}" if not data[col].isna().all() else "N/A"
        })
