...
def save_chart_robust(
    ...
    print(
        f"ℹ️ Parameters: Width={width}, Height={height}, Scale={scale}, Format={file_format}"
    )
    ...
    try:
        if file_format.lower() == "html":
        ...
        ):  # Though not typically a user choice for "image"
            ...
            print(
                f"🔄 Saving as {file_format.upper()} using Plotly's write_image: {output_path}..."
            )
            fig.write_image(
                output_path,
                width=width,
        ...
        if file_format.lower() not in ["html", "json"]:
            ...
            try:
                ...
                print(
                    f"🔍 Full traceback for HTML fallback failure: {traceback.format_exc()}"
                )
...
def test_kaleido_compatibility():
    ...
    try:
        ...
        for fmt in formats_to_test:
            ...
            if success:
                print(
                    f"✅ Test save successful for {fmt.upper()}! Path: {path}, Format Used: {format_used}"
                )
                try:
