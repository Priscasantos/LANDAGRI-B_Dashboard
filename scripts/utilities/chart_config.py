...
EXTENDED_PALETTE = [
    ...
    "#0ea5e9", "#65a30d", "#ea580c", "#e11d48", "#9333ea"
]
...
def get_standard_layout(
    ...
    return {
        ...
        "xaxis": {
            "title": {"text": xaxis_title, "font": {"size": 14, "family": "Inter, sans-serif"}},
            "tickfont": {"size": 12, "family": "Inter, sans-serif"},
            "gridcolor": "rgba(0,0,0,0.1)",
            "linecolor": "rgba(0,0,0,0.2)"
        },
        "yaxis": {
            "title": {"text": yaxis_title, "font": {"size": 14, "family": "Inter, sans-serif"}},
            "tickfont": {"size": 12, "family": "Inter, sans-serif"},
            "gridcolor": "rgba(0,0,0,0.1)",
            "linecolor": "rgba(0,0,0,0.2)"
        },
...
def get_color_palette(n_colors: int, palette: str = "primary") -> list:
    ...
    if palette == "auto" or palette not in MODERN_COLORS:
        if n_colors <= len(EXTENDED_PALETTE):
            ...
            return [EXTENDED_PALETTE[i % len(EXTENDED_PALETTE)] for i in range(n_colors)]
...
def get_chart_size(size_preset: str = "medium") -> Dict[str, Optional[int]]:
    """
