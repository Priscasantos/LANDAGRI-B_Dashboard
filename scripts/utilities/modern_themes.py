...
class ModernThemes:
    ...
    def apply_modern_layout(
        ...
        **kwargs
        ...
        # Handle special parameters that should not go to layout
        special_params = {"show_legend", "legend_title", "custom_margins", "xaxis_tickangle"}
        
        # Handle show_legend separately
        if "show_legend" in kwargs:
            layout_updates["showlegend"] = kwargs["show_legend"]
        
        # Apply other kwargs, excluding special parameters
        filtered_kwargs = {k: v for k, v in kwargs.items() if k not in special_params}
        layout_updates.update(filtered_kwargs)
