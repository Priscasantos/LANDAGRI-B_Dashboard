"""
Detailed Analysis Dashboard Module
==================================

Advanced detailed comparison analysis for LULC initiatives featuring:
- Custom initiative comparisons
...
try:
    ...
    def setup_performance_sidebar():
        st.sidebar.info("⚠️ Cache optimization unavailable")
    ...
    st.sidebar.warning(f"⚠️ Cache system not available: {e}")

# Import chart utilities with error handling
try:
    from scripts.plotting.chart_core import apply_standard_layout
except ImportError:
    apply_standard_layout = None


def run():
    """
    Main detailed analysis function with modern UI.
    """
    # ========================================
    ...
    <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); 
        ...
        <p style="color: rgba(255,255,255,0.9); margin: 0.5rem 0 0 0; font-size: 1.1rem;">
            Custom comparisons and detailed metrics analysis
        </p>
    </div>
    """,
    ...
    if (
        ...
        if OPTIMIZATION_ENABLED:
            st.error("❌ Dados não encontrados na sessão. Usando cache otimizado...")
            metadata, df_original, cache_info = load_optimized_data()
    ...
    if len(selected_initiatives) < 2:
        st.info(
            "👈 Select at least two initiatives in the menu above to start analysis."
        )  # Translated
        return

    # Filter data for selected initiatives
    df_filtered = df_geral[
        df_geral["Name"].isin(selected_initiatives)
    ].copy()  # Standardized tabs with English translations
    ...
    with tab2:
        ...
        radar_columns = [
            ...
            "Classes",
            "Num_Agri_Classes",
        ]  # Fixed column names and added more dimensions
        available_radar_cols = [
            col for col in radar_columns if col in df_filtered.columns
        ]
        ...
        if len(available_radar_cols) >= 3:
            ...
            for col in available_radar_cols:
                ...
                if max_val - min_val > 0:
                    if (
                        col == "Resolution"
                    ):  # Lower is better for resolution, so invert normalization
                        radar_df[col] = 1 - (radar_df[col] - min_val) / (
                            max_val - min_val
                        )
                    ...
                    radar_df[col] = (
                        0.5  # Assign a neutral value if all values are the same
                    )

            fig_radar = go.Figure()
            colors = (
                px.colors.qualitative.Plotly
            )  # Using Plotly colors for better distinction
            ...
            for i, (idx, row) in enumerate(radar_df.iterrows()):
                ...
                theta_closed = available_radar_cols + [
                    available_radar_cols[0]
                ]  # Close the theta
            ...
            fig_radar.update_layout(
                polar=dict(
                    radialaxis=dict(
                        visible=True, range=[0, 1], showticklabels=True, ticks="outside"
                    ),
                    angularaxis=dict(
                        showticklabels=True, rotation=90, direction="counterclockwise"
                    ),
                ),
                showlegend=True,
            ...
            st.warning(
                "⚠️ Only 2 dimensions available. Radar charts work best with 3 or more dimensions. Adding a third dimension for better visualization."
            )
    ...
    with tab3:
        ...
        if (
            radar_df is None and len(available_radar_cols) >= 2
    ...
    with tab5:
        ...
        if not selected_initiatives:
            st.info(
                "Select at least one initiative to view annual coverage."
            )  # Translated
        else:
            try:
    ...
    with tab6:
        ...
        filtered_meta = {
            name: details
            for name, details in meta.items()
            if name in selected_initiatives
        }
        ...
        st.markdown(
            "This section highlights initiatives with missing years in their time series."
        )
                ...
                if len(years) > 1:
                    ...
                    if missing_years:
                        gap_analysis_data.append(
                            {
                                "Initiative": nome_to_sigla.get(
                                    init_name, init_name[:15]
                                ),
                                "Full_Name": init_name,
                                "Total_Years": len(years),
                                "Missing_Years": len(missing_years),
                                "Missing_Years_List": missing_years,
                                "Coverage_Percentage": (len(years) / len(full_range))
                                * 100,
                            }
                        )
        ...
        if gap_analysis_data:
            ...
            with st.expander("📋 Detailed Gap Information"):
                for gap_info in gap_analysis_data:
                    st.markdown(
                        f"**{gap_info['Initiative']}** ({gap_info['Full_Name']})"
                    )
                    st.markdown(f"- Coverage: {gap_info['Coverage_Percentage']:.1f}%")
                    st.markdown(
                        f"- Missing years: {', '.join(map(str, gap_info['Missing_Years_List']))}"
                    )
                    st.markdown("---")
        else:
        ...
        for init_name, details in filtered_meta.items():
            if "available_years" in details and details["available_years"]:
                ...
                init_type = (
                    init_row["Type"].iloc[0]
                    if not init_row.empty and "Type" in init_row.columns
                    else "Unknown"
                )
        ...
        if heatmap_data:
            ...
            with col1:
                ...
                st.metric(
                    "Years Covered",
                    (
                        f"{min(all_years_set)}-{max(all_years_set)}"
                        if all_years_set
                        else "N/A"
                    ),
            ...
            with col2:
                avg_initiatives_per_year = (
                    evolution_df["year"].value_counts().mean()
            ...
            with col3:
                peak_year = (
                    evolution_df["year"].value_counts().idxmax()
                    if not evolution_df.empty
                    else "N/A"
                )
                peak_count = (
                    evolution_df["year"].value_counts().max()
                    if not evolution_df.empty
                    else 0
                )
...
def run_non_streamlit(df, metadata, output_dir="graphics/detailed"):
    ...
    try:
        ...
        selected_initiatives_names = df_for_analysis["Name"].tolist()[
            : min(5, len(df_for_analysis))
        ]
        df_filtered_non_st = df_for_analysis[
            df_for_analysis["Name"].isin(selected_initiatives_names)
        ].copy()
        ...
        if fig_dual_bars:
            success, saved_path, format_used = save_chart_robust(
                ...
                "dual_bars_detailed",
                width=1000,
                height=max(400, len(df_filtered_non_st) * 30 + 150),
                scale=2,  # Adjusted dimensions
            )
            if success:
                print(
                    f"✅ Dual bars saved as {format_used} in: {saved_path}"
                )  # Translated
        ...
        if fig_radar:
            ...
            if success:
                print(
                    f"✅ Radar chart saved as {format_used} in: {saved_path}"
                )  # Translated
        ...
        if fig_heatmap:
            ...
            if success:
                print(
                    f"✅ Heatmap saved as {format_used} in: {saved_path}"
                )  # Translated
        ...
        if metadata:
            ...
            try:
                ...
                fig_annual_coverage = plot_annual_coverage_multiselect(
                    metadata, df_filtered_non_st, selected_initiatives_names
                )
                if fig_annual_coverage:
                    success, saved_path, format_used = save_chart_robust(
                        fig_annual_coverage,
                        output_dir,
                        "annual_coverage_detailed",
                        width=1200,
                        height=700,
                        scale=2,  # Adjusted dimensions
                    )
                    if success:
                        print(
                            f"✅ Annual coverage chart saved as {format_used} in: {saved_path}"
                        )  # Translated
            except ImportError:
                print(
                    "❌ Annual coverage function not available for non-Streamlit execution."
                )  # Translated
            except Exception as e:
...
def create_radar_chart(df_filtered_chart):
    ...
    try:
        ...
        available_radar_cols_chart = [
            col for col in radar_columns if col in df_filtered_chart.columns
        ]
        ...
        radar_df_chart = df_filtered_chart[
            ["Display_Name"] + available_radar_cols_chart
        ].copy()
        for col in available_radar_cols_chart:
            # Convert to numeric and handle NaN values
            radar_df_chart[col] = pd.to_numeric(
                radar_df_chart[col], errors="coerce"
            ).fillna(0)
            min_val, max_val = radar_df_chart[col].min(), radar_df_chart[col].max()
            if max_val - min_val > 0:
                if col == "Resolution":
                    radar_df_chart[col] = 1 - (radar_df_chart[col] - min_val) / (
                        max_val - min_val
                    )
                else:
                    radar_df_chart[col] = (radar_df_chart[col] - min_val) / (
                        max_val - min_val
                    )
            else:
