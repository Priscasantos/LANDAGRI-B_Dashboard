"""
Temporal Analysis Dashboard Module
==================================

Advanced temporal analysis for LULC initiatives featuring:
...
# Import plotting functions with error handling
...
try:
    ...
    st.warning(f"⚠️ Some chart functions not available: {e}")


def run(metadata: Optional[Dict] = None, df_original: Optional[pd.DataFrame] = None):
    """
    Run temporal analysis with modern UI and responsive design.

    Args:
        metadata: Optional metadata dictionary
        df_original: Optional DataFrame for analysis
    """
    # ========================================
    ...
    if OPTIMIZATION_ENABLED:
        ...
        # Carrega dados otimizados se não fornecidos
        if metadata is None or df_original is None:
            metadata_opt, df_opt, cache_info = load_optimized_data()
            if metadata is None:
    ...
    <div style="background: linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%); 
        ...
        <p style="color: rgba(255,255,255,0.9); margin: 0.5rem 0 0 0; font-size: 1.1rem;">
            Comprehensive temporal evolution of LULC initiatives
        </p>
    </div>
    """,
                ...
                if years:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            name=display_name,
                            marker=dict(
                                size=10, color=colors[i % len(colors)], symbol="square"
                            ),
...
def create_basic_coverage_heatmap(temporal_data):
    ...
    try:
        ...
        fig.update_layout(
            ...
            height=400,
            xaxis=dict(title_font=dict(family="Arial Black")),  # Bold x-axis title
            yaxis=dict(title_font=dict(family="Arial Black")),  # Bold y-axis title
        )
    ...
    try:
        ...
        fig = go.Figure(
            data=go.Heatmap(
                ...
                hoverongaps=False,
                hovertemplate="<b>Type: %{y}</b><br>Year: %{x}<br>Active Initiatives: %{z}<extra></extra>",
                colorbar=dict(title="Active<br>Initiatives"),
            )
        )
