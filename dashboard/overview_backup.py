...
def _render_key_metrics(filtered_df: pd.DataFrame, meta: dict = None):
    ...
    Args:
        ...
        meta: Metadata dictionary for additional LULC insights
    """
    st.subheader("📈 LULC Monitoring Overview")
    
    # Coverage type filter buttons
    st.markdown("#### 🌐 Initiative Coverage")
    coverage_col1, coverage_col2, coverage_col3, coverage_col4 = st.columns(4)
    
    # Calculate coverage statistics from metadata if available
    coverage_stats = {"Global": 0, "Regional": 0, "National": 0, "Total": len(filtered_df)}
    
    if meta:
        for name in filtered_df["Name"]:
            if name in meta and "coverage" in meta[name]:
                coverage = meta[name]["coverage"]
                if coverage in coverage_stats:
                    coverage_stats[coverage] += 1
    else:
        # Fallback to Type column if metadata not available
        if "Type" in filtered_df.columns:
            coverage_stats["Global"] = len(filtered_df[filtered_df["Type"] == "Global"])
    ...
    with coverage_col2:
        st.markdown(
            ...
            <div class="metric-card regional">
                ...
                <div class="metric-value">{coverage_stats["Regional"]}</div>
                <div class="metric-label">Regional Initiatives</div>
                <div class="metric-sublabel">Multi-country scope</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with coverage_col3:
        st.markdown(
            ...
            <div class="metric-card national">
                <span class="metric-icon">🏛️</span>
                <div class="metric-value">{coverage_stats["National"]}</div>
                <div class="metric-label">National Initiatives</div>
    ...
    with coverage_col4:
        st.markdown(
            ...
            <div class="metric-card total">
                <span class="metric-icon">📊</span>
                <div class="metric-value">{coverage_stats["Total"]}</div>
                <div class="metric-label">Total Initiatives</div>
                <div class="metric-sublabel">LULC datasets</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with col2:
        ...
        if not avg_resolution_series.empty:
            ...
            resolution_unit = "m" if avg_resolution >= 1 else "km"
            resolution_value = avg_resolution if avg_resolution >= 1 else avg_resolution * 1000
            st.markdown(
                f"""
            <div class="metric-card resolution">
                <span class="metric-icon">🔬</span>
                <div class="metric-value">{resolution_value:.0f}{resolution_unit}</div>
                <div class="metric-label">Average Resolution</div>
                <div class="metric-sublabel">Spatial detail</div>
            </div>
            """,
                unsafe_allow_html=True,
            )
    ...
    with col3:
        ...
        if total_classes > 0:
            ...
            <div class="metric-card classes">
                <span class="metric-icon">🏷️</span>
                <div class="metric-value">{total_classes:.0f}</div>
                <div class="metric-label">Total Classes</div>
                <div class="metric-sublabel">Land cover types</div>
            </div>
            """,
                unsafe_allow_html=True,
            )
    ...
    with col4:
        ...
        if meta:
            all_years = set()
            for name in filtered_df["Name"]:
                if name in meta and "available_years" in meta[name]:
                    years = meta[name]["available_years"]
                    if years:
                        all_years.update(years)
            temporal_coverage = len(all_years)
        
        if temporal_coverage > 0:
            st.markdown(
                f"""
                <div class="metric-card temporal">
                    <span class="metric-icon">📅</span>
                    <div class="metric-value">{temporal_coverage}</div>
                    <div class="metric-label">Years Covered</div>
                    <div class="metric-sublabel">Temporal scope</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def run():
    ...
    try:
        ...
        # All code that uses filtered_df, nome_to_sigla, sensors_meta, etc. goes here
        # ... (move all such code here from below) ...

        # Place the rest of the dashboard logic here (metrics, charts, etc.)
        # For brevity, not duplicating all code, but this is the correct structure

    # End of if df is not None and not df.empty
    """
    ...
    # CSS cleaned up - moving to temporal density chart
    # Broken CSS commented out
