"""
Boxplot Component - Comparison Analysis
======================================

Componente para renderizar a aba de boxplot na análise comparativa.
...
def render_boxplot_tab(filtered_df: pd.DataFrame) -> None:
    """
    Renderizar aba de boxplot para análise comparativa.
    Args:
        filtered_df: DataFrame filtrado com dados das iniciativas
    """
    st.markdown("#### 📦 Boxplot - Distribuição das Métricas por Iniciativa")
...
def create_boxplot_chart(filtered_df: pd.DataFrame) -> px.box:
    """
    Criar boxplot comparativo para múltiplas iniciativas.
    Args:
    ...
    if "Accuracy (%)" in filtered_df.columns:
        fig = px.box(
            ...
            y="Accuracy (%)",
            color="Display_Name",
            title="Distribuição da Acurácia por Iniciativa",
        )
