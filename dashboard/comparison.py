...
try:
    ...
    from scripts.plotting.generate_graphics import (  # Distribution charts for new tabs; New resolution chart imports
    ...
    st.success(
        "SUCCESS: Plotting functions imported from scripts.plotting.generate_graphics!"
    )
    plotting_functions_loaded = True
except ImportError as e_graphics_detail:
    st.error(
        "CRITICAL IMPORT ERROR: Failed to import from scripts.plotting.generate_graphics."
    )
    ...
    def plot_resolution_accuracy_scatter(_filtered_df):
        st.warning(
            "Placeholder: plot_resolution_accuracy_scatter not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_resolution_accuracy_scatter not available", showarrow=False
        )

    def plot_classes_frequency_boxplot(_filtered_df):
        st.warning(
            "Placeholder: plot_classes_frequency_boxplot not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_classes_frequency_boxplot not available", showarrow=False
        )

    def plot_spatial_resolution_comparison(_filtered_df):  # Added placeholder
        st.warning(
            "Placeholder: plot_spatial_resolution_comparison not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_spatial_resolution_comparison not available", showarrow=False
        )

    def plot_global_accuracy_comparison(_filtered_df):  # Added placeholder
        st.warning(
            "Placeholder: plot_global_accuracy_comparison not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_global_accuracy_comparison not available", showarrow=False
        )

    def plot_temporal_evolution_frequency(_filtered_df):  # Added placeholder
        st.warning(
            "Placeholder: plot_temporal_evolution_frequency not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_temporal_evolution_frequency not available", showarrow=False
        )

    def plot_class_diversity_focus(_filtered_df):  # Added placeholder
        st.warning(
            "Placeholder: plot_class_diversity_focus not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_class_diversity_focus not available", showarrow=False
        )

    def plot_classification_methodology(
        filtered_df, chart_type="pie"
    ):  # Corrected placeholder
        st.warning(
            "Placeholder: plot_classification_methodology not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_classification_methodology not available", showarrow=False
        )
    ...
    def plot_resolution_vs_launch_year(filtered_df):
        ...
        return go.Figure().add_annotation(
            text="plot_resolution_vs_launch_year not available", showarrow=False
        )

    def plot_initiativas_by_resolution_category(filtered_df):
        st.warning("Placeholder: plot_iniciativas_by_resolution_category not loaded.")
        return go.Figure().add_annotation(
            text="plot_iniciativas_by_resolution_category not available",
            showarrow=False,
        )
    ...
    def plot_resolution_coverage_heatmap(filtered_df):
        ...
        return go.Figure().add_annotation(
            text="plot_resolution_coverage_heatmap not available", showarrow=False
        )
    ...
    def plot_resolution_by_sensor_family(filtered_df, sensors_meta):
        ...
        return go.Figure().add_annotation(
            text="plot_resolution_by_sensor_family not available", showarrow=False
        )

    def plot_resolution_slopegraph(filtered_df, sensors_meta):
        st.warning("Placeholder: plot_resolution_slopegraph not loaded.")
        return go.Figure().add_annotation(
            text="plot_resolution_slopegraph not available", showarrow=False
        )
    ...
    def plot_distribuicao_classes(filtered_df):
        ...
        return go.Figure().add_annotation(
            text="plot_distribuicao_classes not available", showarrow=False
        )

    def plot_distribuicao_metodologias(method_counts):
        st.warning("Placeholder: plot_distribuicao_metodologias not loaded.")
        return go.Figure().add_annotation(
            text="plot_distribuicao_metodologias not available", showarrow=False
        )

    def plot_acuracia_por_metodologia(filtered_df):
        st.warning("Placeholder: plot_acuracia_por_metodologia not loaded.")
        return go.Figure().add_annotation(
            text="plot_acuracia_por_metodologia not available", showarrow=False
        )

    def plot_normalized_performance_heatmap(
        filtered_df,
    ):  # Added placeholder for the new heatmap
        st.warning(
            "Placeholder: plot_normalized_performance_heatmap not loaded due to import error."
        )
        return go.Figure().add_annotation(
            text="plot_normalized_performance_heatmap not available", showarrow=False
        )
...
def run():
    ...
    with tab_methodology_dist:
        ...
        if not filtered_df.empty:
            try:
                ...
                # or ensure plot_classification_methodology is correctly handling the filtered_df for an overall view.                # Assuming plot_classification_methodology is the correct one for this pre-existing tab.
                methodology_fig = plot_classification_methodology(
                    filtered_df, chart_type=chart_type_param
                )
                st.plotly_chart(
                    methodology_fig,
    ...
    with tab_resolution_analysis:  # Content for the new tab
        ...
        if not plotting_functions_loaded:
            ...
            if "sensors_meta" not in st.session_state:
                try:
                    with open(sensors_data_path, encoding="utf-8") as f:
            ...
            try:
                ...
                st.plotly_chart(
                    fig_res_launch, use_container_width=True, key="res_vs_launch_comp"
                )
            ...
            try:
                fig_res_cat = plot_initiativas_by_resolution_category(filtered_df)
                st.plotly_chart(
            ...
            if not sensors_meta_data:
                ...
                try:
                    fig_res_sensor = plot_resolution_by_sensor_family(
                        filtered_df, sensors_meta_data
                    )
                    st.plotly_chart(
                        fig_res_sensor,
                        use_container_width=True,
                ...
                try:
                    fig_res_slope = plot_resolution_slopegraph(
                        filtered_df, sensors_meta_data
                    )
                    st.plotly_chart(
                        fig_res_slope, use_container_width=True, key="res_slope_comp"
                    )
            ...
            if (
                ...
                try:
                    ...
                    if not df_for_acc_chart.empty:
                        ...
                        st.plotly_chart(
                            fig_acc_meth, use_container_width=True, key="acc_meth_comp"
                        )
    ...
    with (
        ...
        if not plotting_functions_loaded:
            ...
            try:
                fig_normalized_heatmap = plot_normalized_performance_heatmap(
                    filtered_df
                )
                st.plotly_chart(
